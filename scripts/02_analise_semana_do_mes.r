...
if (!file.exists(arquivo_dados)) {
  stop(
    "Base pública não encontrada. Execute primeiro: ",
...
dados <- read_csv(arquivo_dados, show_col_types = FALSE) %>%
  mutate(
    ...
    mes = month(data_dt),
    ano = year(data_dt),
    semana_mes = factor(classificar_semana_mes(dia_mes), levels = ordem_semana),
    marco_do_mes = factor(classificar_marco_mes(dia_mes), levels = ordem_marco)
  )
...
write_csv(resumo_semana, file.path(pasta_tabelas, "indice_por_semana_do_mes.csv"))
write_csv(resumo_marco, file.path(pasta_tabelas, "indice_por_marco_do_mes.csv"))
write_csv(resumo_dia_numero, file.path(pasta_tabelas, "indice_por_dia_numero.csv"))
write_csv(resumo_mes, file.path(pasta_tabelas, "indice_por_mes.csv"))

grafico_semana <- ggplot(
  resumo_semana,
  aes(x = semana_mes, y = indice_medio, fill = cenario)
) +
...
grafico_marco <- ggplot(
  resumo_marco,
  aes(x = marco_do_mes, y = indice_medio, fill = cenario)
) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  geom_text(
    aes(label = round(indice_medio, 1)),
...
grafico_dia <- ggplot(
  resumo_dia_numero,
  aes(x = dia_mes, y = indice_medio, fill = cenario)
) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  geom_hline(yintercept = 100, linetype = "dashed", color = "gray45") +
...
grafico_mes <- ggplot(
  resumo_mes,
  aes(x = ano_mes, y = indice_medio, color = cenario, group = cenario)
) +
  geom_hline(yintercept = 100, linetype = "dashed", color = "gray45") +
...
ggsave(file.path(pasta_graficos, "indice_por_semana_do_mes.png"), grafico_semana, width = 12, height = 7, dpi = 150)
ggsave(file.path(pasta_graficos, "indice_por_marco_do_mes.png"), grafico_marco, width = 12, height = 7, dpi = 150)
ggsave(file.path(pasta_graficos, "indice_por_dia_do_mes.png"), grafico_dia, width = 16, height = 8, dpi = 150)
ggsave(file.path(pasta_graficos, "indice_por_mes.png"), grafico_mes, width = 16, height = 8, dpi = 150)

cat("Análise por semana e marcos do mês concluída.\n")